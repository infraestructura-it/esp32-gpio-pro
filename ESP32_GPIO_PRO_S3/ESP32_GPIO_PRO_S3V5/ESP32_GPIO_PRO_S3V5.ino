...
void        wsHandleControl(DynamicJsonDocument& doc, const char* src, uint8_t client);
...
<div id="appShell" style="display:none">
  ...
  <div class="tabs">
    ...
    <div class="tab" onclick="showTab('ota')" id="tab-ota" style="display:none">OTA</div>
    <div class="tab" onclick="showTab('relay')" id="tab-relay" style="display:none">Relay</div>
  </div>
    ...
    <div class="wrap">
      <div class="card ca">
        ...
        <div class="btn-row">
          ...
          <button class="btn bc" onclick="showTab('ota')" id="otaBtn" style="display:none">ACTUALIZAR FIRMWARE</button>
        </div>
      </div>
  ...
  fetch("/api/login",{method:"POST",headers:{"Content-Type":"application/json"},
    ...
    .then(function(d){
      if(d.ok){
        ...
        if(isAdmin){
          G("tab-ota").style.display="";
          G("otaBtn").style.display="";
        }
        G("tab-mqtt").style.display="block";
        G("tab-net").style.display="block";
        G("tab-relay").style.display="block";
        loadRelayConfig();
        buildGrid();connectWS();
...
// ── OTA ──────────────────────────────────────────────────────────────────
var otaFileObj=null;
function otaFileSelected(inp){
  if(!inp.files||!inp.files[0]) return;
  otaFileObj=inp.files[0];
  G('otaFileName').textContent=otaFileObj.name+' ('+Math.round(otaFileObj.size/1024)+' KB)';
  G('otaUploadBtn').disabled=false;
  G('otaUploadBtn').style.opacity='1';
  otaMsg('','');
}
function otaMsg(txt,type){
  var d=G('otaMsg');
...
function otaSetPill(on,txt){
  var p=G('otaPill'),t=G('otaPillTxt');
  p.className='pill '+(on?'pon':'poff');
  t.textContent=txt;
}
function otaUpload(){
  if(!otaFileObj){otaMsg('Selecciona un archivo .bin primero','err');return;}
  if(!TOKEN){otaMsg('No autenticado','err');return;}
  var xhr=new XMLHttpRequest();
  var prog=G('otaProgress');
  var fill=G('otaProgFill');
  var pct=G('otaProgPct');
  var txt=G('otaProgTxt');
  prog.style.display='block';
  otaSetPill(true,'SUBIENDO');
  G('otaUploadBtn').disabled=true;
  G('otaUploadBtn').style.opacity='.4';
  xhr.upload.onprogress=function(e){
    if(e.lengthComputable){
      var p=Math.round(e.loaded/e.total*100);
      fill.style.width=p+'%';
      pct.textContent=p+'%';
      txt.textContent='Subiendo... '+p+'%';
    }
  };
  xhr.onload=function(){
    if(xhr.status===200){
      ...
      pct.textContent='100%';
      txt.textContent='Completado. Reiniciando...';
      otaSetPill(true,'REINICIANDO');
      otaMsg('Firmware actualizado correctamente. El dispositivo se reiniciara en 5 segundos...','ok');
      setTimeout(function(){
        otaSetPill(false,'LISTO');
        prog.style.display='none';
        G('otaUploadBtn').disabled=false;
        G('otaUploadBtn').style.opacity='1';
        otaFileObj=null;
        G('otaFileName').textContent='Ninguno seleccionado';
      },6000);
    } else {
      otaSetPill(false,'ERROR');
      otaMsg('Error al actualizar: '+xhr.responseText,'err');
      G('otaUploadBtn').disabled=false;
      G('otaUploadBtn').style.opacity='1';
    }
  };
  xhr.onerror=function(){
    otaSetPill(false,'ERROR');
    otaMsg('Error de conexion durante la subida','err');
    G('otaUploadBtn').disabled=false;
    G('otaUploadBtn').style.opacity='1';
  };
  var fd=new FormData();
  fd.append('firmware',otaFileObj,otaFileObj.name);
...
// ── RELAY CONFIG UI ───────────────────────────────────────────────────────
var relayCfgLocal = {host:'',port:443,token:'',tls:true,enabled:false,connected:false,deviceId:''};

function relaySetPill(on, txt){
  var p=G('relayPill'), t=G('relayPillTxt');
  p.className='pill '+(on?'pon':'poff');
  t.textContent = txt || (on?'CONECTADO':'DESCONECTADO');
}

function relayStatusMsg(txt, ok){
  var d=G('relayStatusMsg');
  if(!txt){d.style.display='none';return;}
  d.style.display='block';
  d.style.background=ok?'rgba(57,255,20,.06)':'rgba(255,68,102,.08)';
  d.style.border='1px solid '+(ok?'rgba(57,255,20,.25)':'rgba(255,68,102,.25)');
  d.style.color=ok?'var(--on)':'var(--err)';
  d.textContent=txt;
}

function loadRelayConfig(){
  apiGet('/api/relay/config').then(function(d){
    if(!d) return;
    relayCfgLocal = d;
    G('relayHost').value  = d.host  || '';
    G('relayPort').value  = d.port  || 443;
    G('relayToken').value = d.token || '';
    // TLS switch
    var tlsSw = G('relayTlsSw');
    if(tlsSw){ tlsSw.className='sw'+(d.tls?' on':''); }
    // Master switch
    var msw = G('relayMasterSw');
    if(msw){ msw.className='sw'+(d.enabled?' on':''); }
    // Device ID
    if(d.deviceId) G('relayDevId').textContent = d.deviceId;
    // Pill
    relaySetPill(d.connected, d.connected?'CONECTADO':'DESCONECTADO');
  }).catch(function(){});
}

function saveRelayConfig(){
  var body = {
    host:    G('relayHost').value.trim(),
    port:    parseInt(G('relayPort').value)||443,
    token:   G('relayToken').value.trim(),
    tls:     G('relayTlsSw').className.indexOf(' on')>-1,
    enabled: G('relayMasterSw').className.indexOf(' on')>-1
  };
  apiPost('/api/relay/config', body).then(function(d){
    relayCfgLocal = d;
    relaySetPill(d.connected, d.connected?'CONECTADO':'DESCONECTADO');
    relayStatusMsg('Configuracion guardada', true);
    setTimeout(function(){ relayStatusMsg(''); }, 3000);
  }).catch(function(){ relayStatusMsg('Error al guardar','err'); });
}

function relayToggleTls(){
  var sw=G('relayTlsSw');
  sw.className='sw'+(sw.className.indexOf(' on')>-1?'':' on');
}

function relayMasterToggle(){
  var sw=G('relayMasterSw');
  var on = sw.className.indexOf(' on')===-1;
  sw.className='sw'+(on?' on':'');
  apiPost('/api/relay/enable',{enabled:on}).then(function(d){
    relaySetPill(on, on?'ACTIVADO':'DESACTIVADO');
    relayStatusMsg(on?'Relay activado':'Relay desactivado', true);
    setTimeout(function(){ relayStatusMsg(''); }, 2500);
  }).catch(function(){ relayStatusMsg('Error','err'); });
}

function relayConnectNow(){
  relaySetPill(false,'CONECTANDO...');
  apiPost('/api/relay/connect',{}).then(function(d){
    if(d.ok){
      relaySetPill(true,'CONECTADO');
      relayStatusMsg('Conexion iniciada correctamente', true);
    } else {
      relaySetPill(false,'ERROR');
      relayStatusMsg(d.error||'Error al conectar', false);
    }
    setTimeout(function(){ loadRelayConfig(); }, 2000);
  }).catch(function(){ relaySetPill(false,'ERROR'); relayStatusMsg('Error de conexion', false); });
}

function relayDisconnectNow(){
  apiPost('/api/relay/disconnect',{}).then(function(){
    relaySetPill(false,'DESCONECTADO');
    relayStatusMsg('Relay desconectado', true);
    setTimeout(function(){ relayStatusMsg(''); }, 2500);
  }).catch(function(){ relayStatusMsg('Error','err'); });
}

function copyDevId(){
  var id = G('relayDevId').textContent;
  if(id && id!=='---'){
    navigator.clipboard.writeText(id).then(function(){
      relayStatusMsg('Device ID copiado: '+id, true);
      setTimeout(function(){ relayStatusMsg(''); }, 2500);
    }).catch(function(){
      // fallback
      ...
      relayStatusMsg('Device ID copiado', true);
      setTimeout(function(){ relayStatusMsg(''); }, 2500);
    });
  }
}
...
const char APP_24[] PROGMEM = R"XHTMLX(
  <!-- OTA TAB -->
  <div class="tpage" id="page-ota">
    <div class="wrap">
      <div class="card">
        <div class="ct">Actualizacion de firmware OTA
          <div class="pill poff" id="otaPill"><span class="d"></span><span id="otaPillTxt">LISTO</span></div>
        </div>
        <div style="margin-bottom:16px;color:rgba(200,216,232,.6);font-size:.82rem;line-height:1.6">
          Selecciona el archivo <b>.bin</b> generado por Arduino IDE.<br>
          <span style="color:var(--warn)">&#9888; El dispositivo se reiniciara automaticamente al finalizar.</span>
        </div>
        <div class="cfg-section-title">Archivo de firmware</div>
        <div style="margin:12px 0">
          <input type="file" id="otaFile" accept=".bin" style="display:none" onchange="otaFileSelected(this)">
          <button class="btn bc" onclick="G('otaFile').click()">SELECCIONAR .BIN</button>
          <span id="otaFileName" style="margin-left:12px;font-size:.8rem;color:rgba(200,216,232,.5)">Ninguno seleccionado</span>
        </div>
        <div id="otaProgress" style="display:none;margin:14px 0">
          <div style="font-size:.78rem;color:rgba(200,216,232,.5);margin-bottom:6px" id="otaProgTxt">Subiendo...</div>
          <div style="background:rgba(255,255,255,.06);border-radius:4px;height:8px;overflow:hidden">
            <div id="otaProgFill" style="height:100%;width:0%;background:linear-gradient(90deg,var(--a),#0080ff);transition:width .3s"></div>
          </div>
          <div style="font-size:.78rem;color:var(--a);margin-top:6px;text-align:right" id="otaProgPct">0%</div>
        </div>
        <div id="otaMsg" style="display:none;padding:10px 14px;border-radius:6px;margin-top:12px;font-size:.84rem"></div>
        <div class="btn-row" style="margin-top:14px">
          <button class="btn bg" id="otaUploadBtn" onclick="otaUpload()" disabled style="opacity:.4">SUBIR FIRMWARE</button>
        </div>
      </div>
      <div class="card">
        <div class="ct">Instrucciones</div>
        <div style="font-size:.82rem;color:rgba(200,216,232,.6);line-height:1.8">
          <b>1.</b> En Arduino IDE: <code>Sketch &rarr; Export Compiled Binary</code><br>
          <b>2.</b> Busca el archivo <code>.bin</code> en la carpeta del proyecto<br>
          <b>3.</b> Seleccionalo aqui y presiona <b>SUBIR FIRMWARE</b><br>
          <b>4.</b> Espera el reinicio automatico (~5 seg)<br>
          <b>5.</b> Reconecta al mismo IP despues del reinicio
        </div>
      </div>
    </div>
  </div>
...
const char APP_25[] PROGMEM = R"XHTMLX(
  <!-- RELAY TAB -->
  <div class="tpage" id="page-relay">
    <div class="wrap">
      <div class="card ca">
        <div class="ct">WebSocket Relay P2P
          <div class="pill poff" id="relayPill"><span class="d"></span><span id="relayPillTxt">DESCONECTADO</span></div>
        </div>
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;
          ...
          border-radius:8px;margin-bottom:14px;">
          <div>
            <div style="font-family:var(--or);font-size:0.732rem;color:var(--a);letter-spacing:.1em">RELAY P2P</div>
            <div style="font-family:var(--mo);font-size:0.543rem;color:rgba(200,216,232,.3);margin-top:2px">Control remoto via servidor relay</div>
          </div>
          <div class="sw" id="relayMasterSw" onclick="relayMasterToggle()"></div>
        </div>

        <!-- Servidor -->
        <div class="cfg-section">
          <div class="cfg-section-title">Servidor Relay</div>
          <div class="cfg-row2">
            <div class="cfg-field" style="flex:2">
              <div class="cfg-lbl">Host / IP del relay</div>
              <input class="cfg-inp" id="relayHost" type="text" placeholder="relay.tuservidor.com">
            </div>
            <div class="cfg-field" style="flex:1">
              <div class="cfg-lbl">Puerto</div>
              <input class="cfg-inp" id="relayPort" type="number" placeholder="443" value="443" min="1" max="65535">
            </div>
          </div>
          <div class="cfg-field">
            <div class="cfg-lbl">Token / Clave compartida</div>
            <div style="position:relative">
              <input class="cfg-inp" id="relayToken" type="password" placeholder="clave compartida" style="padding-right:34px">
              <button onclick="toggleEye('relayToken',this)" style="position:absolute;right:8px;top:50%;transform:translateY(-50%);background:none;border:none;color:rgba(200,216,232,.3);cursor:pointer;font-size:1.003rem">&#128065;</button>
            </div>
          </div>
        </div>
        ...
        <div class="cfg-section">
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 0">
            <div>
              <div style="font-size:0.82rem;color:var(--txt)">TLS / HTTPS (wss://)</div>
              <div style="font-family:var(--mo);font-size:0.48rem;color:rgba(200,216,232,.3);margin-top:2px">Activar para Railway, Render y servidores con SSL</div>
            </div>
            <div class="sw" id="relayTlsSw" onclick="relayToggleTls()"></div>
          </div>
        </div>

        <!-- Device ID -->
        <div class="cfg-section">
          <div class="cfg-section-title">Identificador del dispositivo</div>
          <div style="display:flex;align-items:center;gap:10px">
            <div style="font-family:var(--mo);font-size:0.82rem;color:var(--a);letter-spacing:.08em;flex:1" id="relayDevId">---</div>
            <button class="btn bc" onclick="copyDevId()" style="padding:5px 10px;font-size:0.52rem">COPIAR</button>
          </div>
          <div style="font-family:var(--mo);font-size:0.48rem;color:rgba(200,216,232,.3);margin-top:6px">Usa este ID en la app movil para conectarse a este dispositivo</div>
        </div>

        <!-- Botones -->
        <div class="btn-row" style="margin-top:16px">
          <button class="btn bc" onclick="saveRelayConfig()">GUARDAR</button>
          <button class="btn bb" onclick="loadRelayConfig()">RECARGAR</button>
          <button class="btn bg" onclick="relayConnectNow()">CONECTAR</button>
          <button class="btn br" onclick="relayDisconnectNow()">DESCONECTAR</button>
        </div>

        <!-- Status -->
        <div id="relayStatusMsg" style="display:none;margin-top:12px;padding:10px 12px;border-radius:6px;font-size:0.78rem"></div>
      </div>
    </div>
  </div>
...
void sendAppPage(){
  ...
  cl.print(FPSTR(APP_24)); cl.flush();
  cl.print(FPSTR(APP_25)); cl.flush();
}
  ...
  switch(type){
    ...
    case WStype_TEXT:{
      ...
      if(strcmp(cmd,"GET_STATE")==0){
        ...
        // Comandos de control via WS — delegar a wsHandleControl
        wsHandleControl(doc, "ws", client);
      }
      break;
    }
    default: break;
...
void setup(){
  ...
  startNormalMode();
  mqttInit(); relayInit(); registerOTARoutes(); registerP2PRoutes(); registerMqttConfigRoutes(); registerRelayRoutes(); registerNetRoutes(); registerWifiRoutes();
...
// ══════════════════════════════════════════════════════════════════════════
//  WS CONTROL — Comandos de control via WebSocket (local y relay)
// ══════════════════════════════════════════════════════════════════════════
// src: "ws" = cliente local puerto 81, "relay" = via relay externo
// client: numero de cliente WS local (ignorado para relay)

void wsHandleControl(DynamicJsonDocument& doc, const char* src, uint8_t client){
  const char* cmd = doc["cmd"] | "";
  int pin = doc["pin"] | -1;   // número de GPIO (no índice)

  // Buscar índice por número de pin
  int idx = (pin >= 0) ? pinIndex((uint8_t)pin) : -1;

  auto broadcast = [&](String s){
    webSocket.broadcastTXT(s);
    if(strcmp(src,"relay")==0) wsRelay.sendTXT(s);
  };

  // ── GET_STATE ────────────────────────────────────────────────────────
  if(strcmp(cmd,"GET_STATE")==0 || strcmp(cmd,"state")==0){
    String s=buildStateJSON();
    if(strcmp(src,"relay")==0) wsRelay.sendTXT(s);
    else webSocket.sendTXT(client,s);
  ...
  // ── MODE — cambiar modo del pin ───────────────────────────────────────
    ...
    const char* modeStr = doc["mode"] | "none";
    PinMode_t nm = modeFromStr(modeStr);
    if(applyMode(idx,nm)){
      addHist(pin,modeStr,0,src,pinState[idx].name);
      broadcast(buildStateJSON());
    }

  // ── DIGITAL — set HIGH/LOW ────────────────────────────────────────────
  }else if(strcmp(cmd,"digital")==0 && idx>=0){
    if(pinState[idx].mode!=PM_OUTPUT) applyMode(idx,PM_OUTPUT);
    int st = doc["state"] | 0;
    pinState[idx].value=st;
    digitalWrite(PIN_TABLE[idx].pin, st?HIGH:LOW);
    saveGPIOState();
    addHist(pin,"output",st,src,pinState[idx].name);
    broadcast(buildStateJSON());

  // ── ON ────────────────────────────────────────────────────────────────
  }else if(strcmp(cmd,"on")==0 && idx>=0){
    if(pinState[idx].mode!=PM_OUTPUT) applyMode(idx,PM_OUTPUT);
    pinState[idx].value=1;
    digitalWrite(PIN_TABLE[idx].pin,HIGH);
    saveGPIOState();
    addHist(pin,"output",1,src,pinState[idx].name);
    broadcast(buildStateJSON());

  // ── OFF ───────────────────────────────────────────────────────────────
  }else if(strcmp(cmd,"off")==0 && idx>=0){
    if(pinState[idx].mode!=PM_OUTPUT) applyMode(idx,PM_OUTPUT);
    pinState[idx].value=0;
    digitalWrite(PIN_TABLE[idx].pin,LOW);
    saveGPIOState();
    addHist(pin,"output",0,src,pinState[idx].name);
    broadcast(buildStateJSON());

  // ── PWM ───────────────────────────────────────────────────────────────
  }else if(strcmp(cmd,"pwm")==0 && idx>=0){
    if(pinState[idx].mode!=PM_PWM) applyMode(idx,PM_PWM);
    int duty = doc["duty"] | doc["val"] | 0;
    pinState[idx].value=duty;
    ledcWrite(PIN_TABLE[idx].pin,duty);
    saveGPIOState();
    addHist(pin,"pwm",duty,src,pinState[idx].name);
    broadcast(buildStateJSON());

  // ── FREQ ──────────────────────────────────────────────────────────────
  }else if(strcmp(cmd,"freq")==0 && idx>=0){
    uint32_t freq = doc["freq"] | PWM_FREQ_DEF;
    pinState[idx].freq=freq;
    ledcChangeFrequency(PIN_TABLE[idx].pin,freq,PWM_RES);
    saveGPIOState();
    broadcast(buildStateJSON());
  ...
  // ── ALL_OFF ───────────────────────────────────────────────────────────
    ...
    for(int i=0;i<NUM_PINS;i++){
      if(pinState[i].mode==PM_OUTPUT){ pinState[i].value=0; digitalWrite(PIN_TABLE[i].pin,LOW); }
      if(pinState[i].mode==PM_PWM)   { pinState[i].value=0; ledcWrite(PIN_TABLE[i].pin,0); }
    }
    saveGPIOState();
    broadcast(buildStateJSON());
  }
...
// ══════════════════════════════════════════════════════════════════════════
//  OTA — Actualizacion Over The Air por navegador
// ══════════════════════════════════════════════════════════════════════════

void registerOTARoutes(){
  // GET /update — sirve la UI (ya está embebida en el tab OTA de la web)
  // Solo verificamos que el endpoint POST funcione

  server.on("/update", HTTP_POST,
    // onComplete — se llama al finalizar la subida
    [](){
      Session* s = getAuth();
      if(!s){ sendError(401,"No autorizado"); return; }
      if(strcmp(s->role,"admin")!=0){ sendError(403,"Solo admin"); return; }

      if(Update.hasError()){
        String err = "Error OTA: " + String(Update.errorString());
    ...
    // onUpload — se llama por cada chunk recibido
    [](){
      Session* s = getAuth();
      if(!s) return;
      if(strcmp(s->role,"admin")!=0) return;

      HTTPUpload& upload = server.upload();

      if(upload.status == UPLOAD_FILE_START){
        Serial.printf("[OTA] Iniciando: %s (%u bytes)\n",
  ...
  Serial.println(F("[OTA] Rutas registradas -> POST /update"));
}
...
void relayOnMessage(uint8_t* payload, size_t len){
  ...
  Serial.println("[RELAY] RX: "+msg.substring(0,80));
  DynamicJsonDocument doc(1024);
  if(deserializeJson(doc,msg)) return;

  // ── Proxy REST — el relay reenvía peticiones HTTP al ESP32 ────────────
  const char* msgType = doc["type"] | "";
  if(strcmp(msgType,"PROXY_REQ")==0){
    const char* reqId   = doc["reqId"]  | "";
    const char* method  = doc["method"] | "GET";
    const char* path    = doc["path"]   | "/api/info";
    const char* token   = doc["token"]  | "";
    String      body    = doc["body"]   | "";

    // Simular request al servidor interno usando el mismo server
    // Creamos una sesión temporal con el token recibido
    String response = "";
    int    status   = 200;
    ...
    if(!isLogin && !sess){
      ...
      if(strcmp(path,"/api/info")==0){
        ...
        res["uptime_ms"]=(unsigned long)(millis()-bootTime);
        res["ntp_synced"]=ntpSynced;
        res["ws_clients"]=wsClients;
  ...
  // Comandos de control — delegar a wsHandleControl
  wsHandleControl(doc, "relay", 0);
}
