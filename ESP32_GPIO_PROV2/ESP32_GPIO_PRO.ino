/*
 * ESP32 GPIO PRO - OUTPUT / INPUT / PWM / ADC
...
const PinInfo PIN_TABLE[NUM_PINS] = {
  {2,true,true,true,false},{4,true,true,true,false},{5,true,true,true,false},
  {12,true,true,true,false},{13,true,true,true,false},{14,true,true,true,false},
  {15,true,true,true,false},{16,true,true,true,false},{17,true,true,true,false},
  {18,true,true,true,false},{19,true,true,true,false},{21,true,true,true,false},
  {22,true,true,true,false},{23,true,true,true,false},{25,true,true,true,false},
  {26,true,true,true,false},{27,true,true,true,false},{32,true,true,true,true},
  {33,true,true,true,true},{34,false,true,false,true},{35,false,true,false,true},
...
unsigned long relayLastRetry = 0;
...
/* ── RELAY TAB ── */
.qr-wrap{display:flex;flex-direction:column;align-items:center;gap:10px;padding:14px 0;}
.qr-box{width:180px;height:180px;background:#fff;border-radius:10px;
  display:flex;align-items:center;justify-content:center;
  box-shadow:0 0 0 6px rgba(0,255,224,.15),0 4px 20px rgba(0,0,0,.4);}
.qr-box canvas{border-radius:6px;}
.qr-url{font-family:var(--mo);font-size:0.52rem;color:var(--tx3);
  word-break:break-all;text-align:center;max-width:260px;}
.relay-status{display:flex;align-items:center;gap:8px;padding:10px 14px;
)XHTMLX";
const char APP_8[] PROGMEM = R"XHTMLX(
  border-radius:8px;border:1px solid var(--bdr2);background:rgba(128,128,128,.06);}
.relay-dot{width:10px;height:10px;border-radius:50%;background:var(--err);
  transition:background .3s;flex-shrink:0;}
.relay-dot.on{background:var(--on);box-shadow:0 0 8px var(--on);}
.relay-dot.warn{background:var(--warn);}
.relay-stxt{font-family:var(--mo);font-size:0.6rem;color:var(--tx2);}
.relay-id{font-family:var(--or);font-size:0.7rem;color:var(--a);
...
const char APP_14[] PROGMEM = R"XHTMLX(
  ...
  <div class="tpage" id="page-relay">
    <div class="wrap">
      ...
      <!-- Estado de conexion -->
      <div class="card ca">
        <div class="ct">WebSocket Relay
          <div class="pill poff" id="relayPill"><span class="d"></span><span id="relayPillTxt">DESCONECTADO</span></div>
        </div>

        <!-- Toggle master -->
        <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 14px;background:rgba(0,255,224,.04);border:1px solid rgba(0,255,224,.14);border-radius:8px;margin-bottom:14px;">
          <div>
            <div style="font-family:var(--or);font-size:.62rem;color:var(--a);letter-spacing:.1em">RELAY P2P</div>
            <div style="font-family:var(--mo);font-size:.46rem;color:var(--tx3);margin-top:2px">Acceso remoto via servidor intermediario</div>
          </div>
        ...
        <div class="cfg-section">
          ...
          <div class="cfg-row2">
            ...
            <div class="cfg-field">
              <div class="cfg-lbl">Puerto</div>
              <input class="cfg-inp" id="relayPort" type="number" placeholder="8080">
            </div>
          </div>
          <div class="cfg-row2">
            <div class="cfg-field">
              <div class="cfg-lbl">Token secreto</div>
              <input class="cfg-inp" id="relayToken" type="password" placeholder="clave compartida">
            </div>
            <div class="cfg-field">
              <div class="cfg-lbl">TLS (wss://)</div>
              <div class="sw" id="relayTlsSw" onclick="relayToggleTls()" style="margin-top:8px"></div>
            </div>
          </div>
        </div>
        ...
        <div class="btn-row" style="margin-top:4px">
          ...
          <button class="btn bg" id="relayConnBtn" onclick="relayConnect()">CONECTAR</button>
          <button class="btn br" onclick="relayDisconnect()">DESCONECTAR</button>
        </div>
      </div>

      <!-- QR y URL de acceso -->
      <div class="card cb">
        <div class="ct">Acceso Remoto — Codigo QR</div>
        <div class="qr-wrap">
          <div class="qr-box" id="relayQrBox">
            <canvas id="relayQrCanvas"></canvas>
          </div>
          <div class="relay-id" id="relayDevId">---</div>
          <div class="qr-url" id="relayQrUrl">Configura el servidor relay para generar el QR</div>
        </div>

        <!-- Estado detallado -->
        <div class="relay-status" id="relayStatusBox">
          <div class="relay-dot" id="relayDot"></div>
          <div class="relay-stxt" id="relayStatusTxt">Sin conexion al relay</div>
        </div>
      </div>

      <!-- Log -->
      <div class="card">
        <div class="ct">Log Relay</div>
        <div class="log-box" id="relayLog"><span class="le lc">Listo...</span></div>
      </div>
...
// ── RELAY WEBSOCKET ──────────────────────────────────────────────────────
var relayCfg={host:"",port:8080,token:"",tls:false,enabled:false};
var relayTls=false;

function relayLogLine(cls,msg){
  var box=G("relayLog");
  var d=new Date();
  var ts=d.getHours().toString().padStart(2,"0")+":"+d.getMinutes().toString().padStart(2,"0")+":"+d.getSeconds().toString().padStart(2,"0");
  box.insertAdjacentHTML("beforeend","<span class=\"le "+cls+"\">"+ts+" "+msg+"</span>");
  box.scrollTop=box.scrollHeight;
  var lines=box.querySelectorAll(".le");
  if(lines.length>80) lines[0].remove();
}

function loadRelayConfig(){
  apiGet("/api/relay/config").then(function(d){
    relayCfg=Object.assign(relayCfg,d);
    G("relayHost").value  = d.host  || "";
    G("relayPort").value  = d.port  || 8080;
    G("relayToken").value = d.token || "";
    relayTls = d.tls || false;
    if(relayTls) G("relayTlsSw").classList.add("on");
    else         G("relayTlsSw").classList.remove("on");
    if(d.enabled) G("relayMasterSw").classList.add("on");
    else          G("relayMasterSw").classList.remove("on");
    updateRelayQR(d);
    relayLogLine("lc","Config cargada");
  }).catch(function(){ relayLogLine("lr","Error cargando config"); });
}

function saveRelayConfig(){
  var body={
    host:  G("relayHost").value.trim(),
    port:  parseInt(G("relayPort").value)||8080,
    token: G("relayToken").value.trim(),
    tls:   relayTls,
    enabled: G("relayMasterSw").classList.contains("on")
  };
  apiPost("/api/relay/config",body).then(function(d){
    relayCfg=Object.assign(relayCfg,body);
    updateRelayQR(d);
    relayLogLine("lg","Config guardada");
  }).catch(function(){ relayLogLine("lr","Error guardando"); });
}
...
function relayMasterToggle(){
  ...
  var en=!sw.classList.contains("on");
  apiPost("/api/relay/enable",{enabled:en}).then(function(){
    if(en) sw.classList.add("on"); else sw.classList.remove("on");
    relayLogLine(en?"lg":"lw", en?"Relay activado":"Relay desactivado");
  });
}

function relayConnect(){
  apiPost("/api/relay/connect",{}).then(function(){
    relayLogLine("lg","Conectando al relay...");
  }).catch(function(){ relayLogLine("lr","Error al conectar"); });
}

function relayDisconnect(){
  apiPost("/api/relay/disconnect",{}).then(function(){
    relayLogLine("lw","Desconectado del relay");
  }).catch(function(){ relayLogLine("lr","Error al desconectar"); });
}

function updateRelayQR(d){
  var host  = (d && d.host)  ? d.host  : relayCfg.host;
  var port  = (d && d.port)  ? d.port  : relayCfg.port;
  var devId = (d && d.deviceId) ? d.deviceId : G("relayDevId").textContent;
  var tls   = (d && d.tls)   ? d.tls   : relayTls;
  if(!host || !devId || devId==="---"){
    G("relayQrUrl").textContent="Configura el servidor relay para generar el QR";
    return;
  }
  var proto = tls ? "https" : "http";
  var url   = proto+"://"+host+(port&&port!==80&&port!==443?":"+port:"")+"/control?id="+devId;
  G("relayQrUrl").textContent = url;
  G("relayDevId").textContent = devId;
  drawQR(url);
}

function drawQR(text){
  var box=G("relayQrBox");
  if(!box) return;
  box.innerHTML="";
  var div=document.createElement("div");
  box.appendChild(div);
  try{
    new QRCode(div,{text:text,width:160,height:160,
      colorDark:"#000000",colorLight:"#ffffff",
      correctLevel:QRCode.CorrectLevel.M});
  } catch(e){
    box.textContent="QR: "+text;
  }
}

// Actualizar estado del relay via WebSocket
function updateRelayStatus(data){
  if(!data || !data.relay) return;
  var r=data.relay;
  var pill=G("relayPill");
  var dot=G("relayDot");
  var txt=G("relayStatusTxt");
  if(!pill||!dot||!txt) return;
  if(r.connected){
    pill.className="pill pon";
    G("relayPillTxt").textContent="CONECTADO";
    dot.className="relay-dot on";
    txt.textContent="Conectado a "+r.host+" | ID: "+r.deviceId;
    if(r.deviceId) G("relayDevId").textContent=r.deviceId;
  } else if(r.enabled){
    pill.className="pill pwarn";
    G("relayPillTxt").textContent="CONECTANDO...";
    dot.className="relay-dot warn";
    txt.textContent="Intentando conectar...";
  } else {
    pill.className="pill poff";
    G("relayPillTxt").textContent="DESCONECTADO";
    dot.className="relay-dot";
    txt.textContent="Relay deshabilitado";
  }
}
...
void setup(){
  ...
  strncpy(pinState[0].name,"LED onboard",23);
  ...
  startNormalMode();
  mqttInit(); registerP2PRoutes(); registerMqttConfigRoutes(); registerRelayRoutes();
  relayInit(); registerNetRoutes(); registerWifiRoutes();
  ...
  if(savedSSID.length()>0 && userCount>0){
    ...
    if(connectWiFi()){
      ...
      mqttInit(); registerP2PRoutes(); registerMqttConfigRoutes(); registerNetRoutes(); registerWifiRoutes();
...
void loop(){
  ...
  if(appMode==MODE_PROVISION){
    ...
    server.handleClient(); webSocket.loop(); checkScheduler(); mqttLoop();
#if !DEBUG_MODE
    if(WiFi.status()!=WL_CONNECTED){
...
// ════════════════════════════════════════════════════════════════════════
// RELAY WEBSOCKET — cliente que se conecta al servidor intermediario
// ════════════════════════════════════════════════════════════════════════
...
void relayOnMessage(uint8_t* payload, size_t len){
  ...
  if(strcmp(cmd,"on")==0 && pinIdx>=0 && pinIdx<NUM_PINS){
    ...
    digitalWrite(pinState[pinIdx].pin==255?0:PIN_TABLE[pinIdx].pin,HIGH);
    addHistory(pinIdx,"OUTPUT",1,"relay");
    webSocket.broadcastTXT(buildStateJSON());
  } else if(strcmp(cmd,"off")==0 && pinIdx>=0 && pinIdx<NUM_PINS){
    pinState[pinIdx].value=0;
    digitalWrite(PIN_TABLE[pinIdx].pin,LOW);
    addHistory(pinIdx,"OUTPUT",0,"relay");
    webSocket.broadcastTXT(buildStateJSON());
  } else if(strcmp(cmd,"pwm")==0 && pinIdx>=0 && pinIdx<NUM_PINS){
    int val = doc["val"] | 0;
    pinState[pinIdx].value=val;
    ledcWrite(pinState[pinIdx].ledCh, val);
    addHistory(pinIdx,"PWM",val,"relay");
    webSocket.broadcastTXT(buildStateJSON());
  } else if(strcmp(cmd,"state")==0){
    wsRelay.sendTXT(buildStateJSON());
  }
}
...
void relayWebSocketEvent(WStype_t type, uint8_t* payload, size_t length){
  switch(type){
    case WStype_CONNECTED:
      ...
      // Presentarse con el deviceId y token
      {
...
void relayInit(){
  // Generar deviceId desde MAC del ESP32
